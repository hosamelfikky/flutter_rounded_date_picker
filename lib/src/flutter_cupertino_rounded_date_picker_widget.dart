...
class _DatePickerLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    for (int i = 0; i < columnWidths.length; i++) {
      ...
      if (index == 0 || index == columnWidths.length - 1) childWidth += remainingWidth / 2;
    ...
    switch (columnType) {
      ...
      case _PickerColumnType.month:
        for (int i = 1; i <= 12; i++) {
          ...
          if (longestText.length < month.length) longestText = month + "_____"; // Add more length of month name for Thai tone marks
        }
        break;
...
class _CupertinoDatePickerDateTimeState extends State<FlutterRoundedCupertinoDatePickerWidget> {
  ...
  double _getEstimatedColumnWidth(_PickerColumnType columnType) {
    if (estimatedColumnWidths[columnType.index] == null) {
      estimatedColumnWidths[columnType.index] = FlutterRoundedCupertinoDatePickerWidget._getColumnWidth(columnType, localizations, context);
    }
  ...
  Widget _buildMediumDatePicker(double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    return CupertinoPicker.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        if (widget.minimumDate != null && dateTime.isBefore(widget.minimumDate!)) return null;
        if (widget.maximumDate != null && dateTime.isAfter(widget.maximumDate!)) return null;
        ...
        return itemPositioningBuilder(
          ...
          Text(
            dateText,
            style: _themeTextStyle(context).copyWith(fontFamily: widget.fontFamily, color: widget.textColor),
          ),
        );
      },
    );
...
class _CupertinoDatePickerDateState extends State<FlutterRoundedCupertinoDatePickerWidget> {
  late int textDirectionFactor;
  late CupertinoLocalizations localizations;
  ...
  void didChangeDependencies() {
    ...
    estimatedColumnWidths[_PickerColumnType.dayOfMonth.index] =
        FlutterRoundedCupertinoDatePickerWidget._getColumnWidth(_PickerColumnType.dayOfMonth, localizations, context);
    estimatedColumnWidths[_PickerColumnType.month.index] =
        FlutterRoundedCupertinoDatePickerWidget._getColumnWidth(_PickerColumnType.month, localizations, context);
    estimatedColumnWidths[_PickerColumnType.year.index] =
        FlutterRoundedCupertinoDatePickerWidget._getColumnWidth(_PickerColumnType.year, localizations, context);
  }
  ...
  Widget _buildDayPicker(double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    ...
    return CupertinoPicker(
      ...
      onSelectedItemChanged: (int index) {
        selectedDay = index + 1;
        if (DateTime(selectedYear, selectedMonth, selectedDay).day == selectedDay) widget.onDateTimeChanged(DateTime(selectedYear, selectedMonth, selectedDay));
      },
      children: List<Widget>.generate(31, (int index) {
        ...
        return itemPositioningBuilder(
          ...
          Text(
            ...
            style: textStyle.copyWith(fontFamily: widget.fontFamily, color: widget.textColor),
          ),
        );
      }),
      looping: true,
  ...
  Widget _buildMonthPicker(double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    return CupertinoPicker(
      ...
      children: List<Widget>.generate(12, (int index) {
        return itemPositioningBuilder(
          context,
          Text(localizations.datePickerMonth(index + 1), style: _themeTextStyle(context).copyWith(fontFamily: widget.fontFamily, color: widget.textColor)),
        );
      }),
      looping: true,
    );
  ...
  Widget _buildYearPicker(double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    return CupertinoPicker.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        if (widget.maximumYear != null && index > widget.maximumYear!) return null;
...
class _CupertinoTimerPickerState extends State<FlutterRoundedCupertinoDurationPickerWidget> {
  late int textDirectionFactor;
  late CupertinoLocalizations localizations;
  ...
  Widget _buildLabel(String text) {
    return Text(
      text,
      textScaleFactor: 0.9,
      style: TextStyle(fontWeight: FontWeight.w600, fontFamily: widget.fontFamily, color: widget.textColor),
    );
  }
  ...
  Widget _buildHourPicker() {
    return CupertinoPicker(
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          selectedHour = index;
          widget.onTimerDurationChanged(Duration(hours: selectedHour!, minutes: selectedMinute, seconds: selectedSecond ?? 0));
        });
      },
      children: List<Widget>.generate(24, (int index) {
        final double hourLabelWidth = widget.mode == CupertinoTimerPickerMode.hm ? _kPickerWidth / 4 : _kPickerWidth / 6;
        ...
        return Semantics(
          ...
          child: Container(
            alignment: alignCenterRight,
            padding: textDirectionFactor == 1 ? EdgeInsets.only(right: hourLabelWidth) : EdgeInsets.only(left: hourLabelWidth),
            child: Container(
              alignment: alignCenterRight,
    ...
    final Widget hourLabel = IgnorePointer(
      child: Container(
        ...
        child: Container(
          ...
          // Adds some spaces between words.
          padding: const EdgeInsets.symmetric(horizontal: 2.0),
          width: widget.mode == CupertinoTimerPickerMode.hm ? _kPickerWidth / 4 : _kPickerWidth / 6,
  ...
  Widget _buildMinutePicker() {
    ...
    return CupertinoPicker(
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          selectedMinute = index * widget.minuteInterval;
          widget.onTimerDurationChanged(Duration(hours: selectedHour ?? 0, minutes: selectedMinute, seconds: selectedSecond ?? 0));
        });
      },
      children: List<Widget>.generate(60 ~/ widget.minuteInterval, (int index) {
        final int minute = index * widget.minuteInterval;
        ...
        if (widget.mode == CupertinoTimerPickerMode.ms) {
          return Semantics(
            ...
            child: Container(
              alignment: alignCenterRight,
              padding: textDirectionFactor == 1 ? const EdgeInsets.only(right: _kPickerWidth / 4) : const EdgeInsets.only(left: _kPickerWidth / 4),
              child: Container(
                alignment: alignCenterRight,
                padding: const EdgeInsets.symmetric(horizontal: 2.0),
                child: Text(localizations.timerPickerMinute(minute), style: TextStyle(fontFamily: widget.fontFamily, color: widget.textColor)),
              ),
            ),
          );
        } else {
          return Semantics(
            ...
            child: Container(
              ...
              child: Container(
                alignment: alignCenterRight,
                width: widget.mode == CupertinoTimerPickerMode.hm ? _kPickerWidth / 10 : _kPickerWidth / 6,
                // Adds some spaces between words.
                padding: const EdgeInsets.symmetric(horizontal: 2.0),
                child: Text(localizations.timerPickerMinute(minute), style: TextStyle(fontFamily: widget.fontFamily, color: widget.textColor)),
              ),
  ...
  Widget _buildMinuteColumn() {
    ...
    if (widget.mode == CupertinoTimerPickerMode.hm) {
      minuteLabel = IgnorePointer(
        child: Container(
          alignment: alignCenterLeft,
          padding: textDirectionFactor == 1 ? const EdgeInsets.only(left: _kPickerWidth / 10) : const EdgeInsets.only(right: _kPickerWidth / 10),
          child: Container(
            alignment: alignCenterLeft,
            // Adds some spaces between words.
            padding: const EdgeInsets.symmetric(horizontal: 2.0),
      ...
      minuteLabel = IgnorePointer(
        child: Container(
          ...
          child: Container(
            alignment: alignCenterLeft,
            width: widget.mode == CupertinoTimerPickerMode.ms ? _kPickerWidth / 4 : _kPickerWidth / 6,
            // Adds some spaces between words.
            padding: const EdgeInsets.symmetric(horizontal: 2.0),
            child: _buildLabel(localizations.timerPickerMinuteLabel(selectedMinute)!),
          ),
  ...
  Widget _buildSecondPicker() {
    ...
    final double secondPickerWidth = widget.mode == CupertinoTimerPickerMode.ms ? _kPickerWidth / 10 : _kPickerWidth / 6;
    ...
    return CupertinoPicker(
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          ...
          widget.onTimerDurationChanged(Duration(hours: selectedHour ?? 0, minutes: selectedMinute, seconds: selectedSecond!));
        });
      },
        ...
        return Semantics(
          ...
          child: Container(
            ...
            child: Container(
              ...
              child: Text(localizations.timerPickerSecond(second), style: TextStyle(fontFamily: widget.fontFamily, color: widget.textColor)),
            ),
          ),
        );
      }),
  ...
  Widget _buildSecondColumn() {
    ...
    final Widget secondLabel = IgnorePointer(
      child: Container(
        alignment: alignCenterLeft,
        padding: textDirectionFactor == 1 ? EdgeInsets.only(left: secondPickerWidth) : EdgeInsets.only(right: secondPickerWidth),
        child: Container(
          alignment: alignCenterLeft,
          // Adds some spaces between words.
          padding: const EdgeInsets.symmetric(horizontal: 2.0),
